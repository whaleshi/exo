...
const List = () => {
    ...
    const { data: contractData, isLoading: contractLoading, isFetching, error } = useQuery({
        ...
        queryFn: async () => {
            ...
            if (tokenCount === 0 || startIndex >= tokenCount) {
                return { tokenCount, tokens: [] };
            }
            const endIndex = Math.min(tokenCount, startIndex + ITEMS_PER_PAGE);

            // 使用 Multicall 批量获取当前页代币地址
            const multicallContract = new ethers.Contract(MULTICALL3_ADDRESS, MULTICALL3_ABI, provider);

            // 准备 multicall 调用获取地址（当前页范围）
            const addressCalls = [];
            ...
            console.log(`Executing Multicall for addresses index [${startIndex}, ${endIndex}) ...`);
            let addressResults;
            try {
            ...
            addressResults.forEach((result: any, index: number) => {
                if (result.success) {
                    try {
                        ...
                        console.log(`Token ${startIndex + index}: ${tokenAddress}`);
                    } catch (error) {
            ...
            console.log(`Executing Multicall for ${validAddresses.length} tokens data (page ${page})...`);
            let dataResults;
            try {
    ...
    // 顶部派生列表（当前页数据内进行过滤和排序）
    const filteredList = useMemo(() => getFilteredTokenList(tokenList, searchQuery), [tokenList, searchQuery]);
    const sortedList = useMemo(() => getSortedTokenList(filteredList, active), [filteredList, active]);

    // 总条数与总页数来自链上统计
    const totalCount = contractData?.tokenCount ?? 0;
    const totalPages = Math.max(1, Math.ceil(totalCount / ITEMS_PER_PAGE));

    // 搜索或 Tab 变化时回到第 1 页（注意：此为页面级分页，搜索仅作用于当前页数据）
    useEffect(() => { setPage(1); }, [searchQuery, active]);
    // 当页码超过最大页时回退
    useEffect(() => {
